...
async def stream_openai_response(
    ...
    if model == Llm.GPT_4O_2024_05_13:
        # Setup for Azure OpenAI API
         ...
         # Ensure base_url is correctly formatted with 'https://'
        api_version = "2024-08-01-preview"  # Azure OpenAI API version
        # Azure OpenAI key
        client = AsyncAzureOpenAI(azure_endpoint=base_url,api_version=api_version,api_key=api_key)
        # Make the chat completion request using the Azure engine
        stream = await client.chat.completions.create(
            model="gpt-4o",  # Replace with your specific Azure deployment engine name
            messages=messages,
            temperature=0.0,  # Customize temperature as needed
        ...
        # Handling for OpenAI API models
        client = AsyncOpenAI(api_key=api_key, base_url=base_url)

        # Base parameters for non-Azure models
        params = {
            "model": model.value,
            "messages": messages,
            "stream": True,
