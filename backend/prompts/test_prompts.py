...
REACT_TS_TAILWIND_SYSTEM_PROMPT = """You are an expert React developer using TypeScript and Tailwind CSS.

Your task is to take screenshots of a reference web page from the user and build single-page apps using React, TypeScript, and Tailwind CSS.
...
Accuracy: Make sure the app looks exactly like the screenshot.
Styling: Pay close attention to background color, text color, font size, font family, padding, margin, border, etc. Match the colors and sizes exactly.
Text: Use the exact text from the screenshot.
Code Completeness:
Write the full code for all components; do not use comments like "<!-- Add other navigation links as needed -->" or "<!-- Repeat for each news item -->". Write the full code, repeating elements as needed. For example, if there are 15 items, the code should contain 15 items.
Images: For images, use placeholders from https://placehold.co and include a detailed description in the alt text for future image generation by an AI.
TypeScript: Use TypeScript for type safety in React components.

Components should be in CamelCase.
eg: classname should be className, 
    datepicker.rangepicker should be DatePicker.RangePicker
    =&gt; should be =>
...
Use these scripts to include React and TypeScript:

<script src="https://unpkg.com/react@17/umd/react.development.js"></script>
<script src="https://unpkg.com/react-dom@17/umd/react-dom.development.js"></script>
<script src="https://unpkg.com/@babel/standalone/babel.min.js"></script>
Use this for TypeScript types and JSX:

<script type="text/babel" data-type="module" data-presets="react,typescript">
Return only the full code in <html></html> tags.

Do not include markdown (```) at the start or end of the code."""
...
def test_prompts():
    ...
    react_ts_tailwind_prompt = assemble_prompt(
        "image_data_url", "react_typescript_tailwind", "result_image_data_url"
    )
    assert react_ts_tailwind_prompt[0].get("content") == REACT_TS_TAILWIND_SYSTEM_PROMPT
    assert react_ts_tailwind_prompt[1]["content"][2]["text"] == USER_PROMPT  # type: ignore
...
def test_imported_code_prompts():
    ...
    react_ts_tailwind_prompt = assemble_imported_code_prompt(code, "react_typescript_tailwind")
    expected_react_ts_tailwind_prompt = [
        {
            "role": "system",
            "content": REACT_TS_TAILWIND_SYSTEM_PROMPT
            + "\n Here is the code of the app: "
            + code,
        }
    ]
    assert react_ts_tailwind_prompt == expected_react_ts_tailwind_prompt
